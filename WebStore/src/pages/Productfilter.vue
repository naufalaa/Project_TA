      ...
      <div class="col-10">
        <div class="item-center">
          <q-avatar rounded size="84px">
            <q-img src="~assets/filter.png" />
          </q-avatar>
          <span class="text-h6 text-black">Pencarian</span>
          <span class="text-h6 text-primary q-pt-md">&nbsp;{{ this.$route.params.filter}}</span>
        </div>
        <q-table
          v-if="dataProduct"
          grid
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      dataProduct: (state) => state.products.productfilter
    }),
    ...mapGetters({
      dataUser: 'auth/user',
  ...
  watch: {
    '$route.params.filter': function (id) {
      this.$store.dispatch('products/productfilter', this.$route.params.filter)
    }
  },
  mounted () {
    this.$store.dispatch('products/productfilter', this.$route.params.filter)
